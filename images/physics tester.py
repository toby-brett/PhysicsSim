...
def collision():
    if pygame.Rect.colliderect(collide_class.rect, cube_class.rect):
        if cube_class.rect.x < collide_class.rect.x:            
            passive_push_force = cube_class.momentum
...
while True:
    ...
    for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if cube_class.rect.collidepoint(event.pos):
                    cube_class.active = True
                    collide_class.active = False
                if collide_class.rect.collidepoint(event.pos):
                    collide_class.active = True
                    cube_class.active = False

                print(event.pos, collide_class.rect)
   ...
   # all text
    ...
    collide_class.changing_variables() 
    collide_class.movement_active()
    collide_class.movement_passive()
    if vectors == 1:
        cube_class.vector_lines()
        collide_class.vector_lines()
